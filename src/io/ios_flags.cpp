...
namespace lf {
    ...
    const std::vector<std::pair<std::ios_base::openmode, const char*>> ios_flag_names = {
        ...
        { std::ios_base::app, "append" }, // seek to the end of stream before each write
        { std::ios_base::trunc, "truncate" }, // discard the contents of the stream when opening
        { std::ios_base::ate, "at-end" }, // seek to the end of stream immediately after open
